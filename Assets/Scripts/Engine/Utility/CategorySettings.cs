...
public static class CategorySettings
{
    ...
    private static void UpdateCategoryCount(Transform transform,Transform categoryObject, string category) {
        ...
        CategoryObjectDestroyListener listener = transform.gameObject.AddComponent<CategoryObjectDestroyListener>();
        listener.onDestroy += delegate(Transform parent, Transform child) {
            string[] transNames = parent.name.Split('[');
            parent.name = string.Format("{0} [{1}]", transNames[0].Trim()
                , parent.childCount-1);
        };
        categoryObject.name = string.Format("{0} [{1}]", tempName, categoryObject.childCount);
    }
    #endregion
    ...
    private static Dictionary<string, Transform> m_CategoryMap = new Dictionary<string, Transform>();
    #endregion
}
